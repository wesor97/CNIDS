...
#include "predictionSnort/AlertNetwork.h"
...
void FastLogger::alert(Packet* p, const char* msg, const Event& event)
{
    ...
    AlertNetwork alert (fast_log);
    alert.make_log(event.sig_info->sid,event.sig_info->gid,snort::SnortConfig::get_networkRule());
...
{
    bool log_pkt = true;SnortConfig::get_networkRule();
