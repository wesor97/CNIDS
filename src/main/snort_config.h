...
public:
    ...
    /*Para tenerla siempre disponible creamos la red y la guardamos aquí*/
    Network* rule_network;
    list<ElementRule> list_element;
    ...
    void set_networkRule(Network* g);
    void set_listElement(list<ElementRule> listilla);
    void free_rule_state_list();

    //------------------------------------------------------
    // Static convenience accessor methods
    
    static Network* get_networkRule()
    { return get_conf()->rule_network; }
    
    static list<ElementRule> get_listElement()
    { return get_conf()->list_element; }
