...
#include "predictionSnort/ElementRule.h"
#include "predictionSnort/InteractionRule.h"
...
/*Metodo para crear la red compleja y guardarla en SnortConfig*/
void ParseNetworkFile(SnortConfig* sc, const char*fname)
{	
		parseRuleNetwork prn(fname);
        	list<ElementRule> lista1 = prn.parseRules();
		if(sc->get_listElement().size()==0){
			sc->set_listElement(lista1);
		}
		else{
			list<ElementRule> lista = sc->get_listElement();
			for(ElementRule e : lista){
				lista1.push_back(e);
			}
			sc->set_listElement(lista1);
		}
